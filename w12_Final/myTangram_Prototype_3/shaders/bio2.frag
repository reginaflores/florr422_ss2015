...
float capsules(vec3 p) {
    vec3 q = floor(p/4.0);
    p = mod(p, 4.0) - 2.0;
    p.yz = p.yz*cos(u_time + q.z) + vec2(-p.z, p.y)*sin(u_time + q.z);
    p.xy = p.xy*cos(u_time + q.x) + vec2(-p.y, p.x)*sin(u_time + q.x);
    p.zx = p.zx*cos(u_time + q.y) + vec2(-p.x, p.z)*sin(u_time + q.y);
    
    float angle = .3*cos(u_time)*p.x;
...
float cubeMap(vec3 p, vec3 n) {
    float a = texture2D(u_tex0, p.yz).r;
    float b = texture2D(u_tex0, p.xz).r;
    float c = texture2D(u_tex0, p.xy).r;
    n = abs(n);
...
void main() {
    ...
    vec3 ro = vec3(u_time, u_time, cos(u_time));
    vec3 rd = normalize(lookat(ro, ro+vec3(cos(.1*u_time), sin(.1*u_time), 1.0))*vec3(uv, -1.0)); // direção do raio.
    
    // based on eiffie's antialiasing method (https://www.shadertoy.com/view/XsSXDt)
    vec3 col = vec3(0.0);
    vec4 stack = vec4(-1.0); bool grab = true;
    float t = 0.0, d = EPS, od = d, pix = 4.0/u_resolution.x, w = 1.8, s = 0.0;
