...
void main() {
    ...
    st *= vec2(1000.,1000.); //1000 cols and 1000 rows
    vec2 i_st = floor(st);
    vec2 f_st = fract(st);
    ...
    vec2 vel = vec2(u_time*max(1000.,1000.)); // time
    vel *= vec2(1.,0.0) * random(0.5+i_st.y); // direction
	color.r = pattern(st,vel,0.5+u_mouse.x/u_resolution.x);
    color.g = pattern(st,vel,0.5+u_mouse.x/u_resolution.x);
    color.b = pattern(st,vel,0.5+u_mouse.x/u_resolution.x);
    //
