...
void main( void ) {
	...
	nd =sin(3.17*0.8*pos.x + (i*0.1+sin(+u_time)*0.4) + u_time)*0.4+0.1 + pos.x;
	...
	nd =sin(3.14*2.0*pos.y + i*40.5 + u_time)*90.3*(pos.y+80.3)+0.5;
  ...
  gl_FragColor = cbuff + dbuff;
	...
	float anim_u_time = clamp(u_time * 0.25, 0.0, 1.0) * 46.0;
